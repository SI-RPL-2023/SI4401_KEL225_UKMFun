<nav class="navbar navbar-expand-lg bg-light shadow fixed-top">
    <div class="container">
        <a class="navbar-brand" href="#">
            <img src="{{ $pengaturan->logo() }}" class="my-2" alt="tel-u" height="40px">
        </a>
        <div class="navbar-nav">
            <ul class="nav justify-content-end">
                @if (auth()->user()->role === 'mahasiswa')
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'Home' ? 'text-danger' : 'text-dark' }}"
                            href="/home">HOME</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'Event' ? 'text-danger' : 'text-dark' }}"
                            href="/event-telutizen">EVENT</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'UKM' ? 'text-danger' : 'text-dark' }}" href="/ukm">UKM</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'About Us' ? 'text-danger' : 'text-dark' }}"
                            href="/about">ABOUT
                            US</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link text-dark" href="/notifikasi" data-bs-toggle="modal"
                            data-bs-target="#exampleModal"><img class="me-3"
                                src="{{ asset('asset/img/notifikasi.png') }}" alt="" height="20px"></a>
                    </li>
                @elseif (auth()->user()->role === 'ukm')
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === auth()->user()->nama ? 'text-danger' : 'text-dark' }}"
                            href="/home-ukm">{{ auth()->user()->nama }}</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'Edit' ? 'text-danger' : 'text-dark' }}"
                            href="/edit">EDIT</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'Event' ? 'text-danger' : 'text-dark' }}"
                    ...
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'Pendaftaran' ? 'text-danger' : 'text-dark' }}"
                            href="/pendaftaran">PENDAFTARAN</a>
                    </li>
                @elseif (auth()->user()->role === 'general admin')
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'Dashboard' ? 'text-danger' : 'text-dark' }}"
                            href="{{ route('dashboard') }}">DASHBOARD</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'UKM FUN' ? 'text-danger' : 'text-dark' }}"
                            href="{{ route('ukm-fun-admin') }}">UKM FUN</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'EDIT CONTENT' ? 'text-danger' : 'text-dark' }}"
                            href="{{ route('pengaturan.index') }}">EDIT CONTENT</a>
                    </li>
                    <li class="nav-item mx-2">
                        <a class="nav-link {{ $title === 'USER' ? 'text-danger' : 'text-dark' }}"
